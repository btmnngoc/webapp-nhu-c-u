...
def tong_hop_nhu_cau(don_hang, sua_chua, ma_san_pham, granularity='M'):
    ...
    if granularity == 'M':
        ...
        max_date = pd.Timestamp('2025-09-01')  # Bắt đầu từ tháng sau ngày hiện tại
    else:  # 'W'
        period_col = 'Week'
        freq = 'W'
        lags = [1, 2, 3, 4, 52]
        ...
        max_date = pd.Timestamp('2025-08-25')  # Bắt đầu từ tuần sau ngày hiện tại
    ...
    nhu_cau_agg['y'] = pd.to_numeric(nhu_cau_agg['Quantity_don_hang'].fillna(0) + nhu_cau_agg['Quantity_sua_chua'].fillna(0), errors='coerce').fillna(0)
